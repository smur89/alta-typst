...
#let _header(
  ...
  context {
    ...
    // cryptic `image()` failure or a silent missing photo. Validation
    // runs even under `anonymous` so a malformed `basics.image` still
    // surfaces — the toggle only suppresses rendering, never validation.
