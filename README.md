...
| `linkContactInfo` | `true` | When true, the contact bar wraps each entry in a deep link (`mailto:`, `tel:`, Google Maps for location, supplied URL for profiles). When false, the icons stay but the values render as plain text — useful for print-only CVs. |
