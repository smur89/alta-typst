...
// for volunteer).
#let _entry_section(entries, heading, org-key, labels, prefs) = if (
  entries.len() > 0
) [
  == #heading
  ...
  #_join_with_dividers(entries, entry => [
    #block(breakable: false)[
      ...
      #term(_format_date_range(entry, prefs, labels), location: entry.at(
        "location",
        default: none,
      ))
      ...
      #let preamble = if _present(summary) { summary } else {
        entry.at("description", default: none)
      }
...
#let _experience(work, labels, prefs) = _entry_section(
  work,
  labels.work,
  "name",
  labels,
...
// behaviour).
#let _volunteer(entries, labels, prefs) = _entry_section(
  entries,
  labels.volunteer,
  "organization",
  labels,
