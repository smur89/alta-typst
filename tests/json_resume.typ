// from-json-resume: smoke test that a JSON Resume document with
// altacv's three extensions (focusAreas, languages[].rating,
// publications[].type) parses and lands in the shape `alta` expects.
// Dates must be iso8601 (YYYY / YYYY-MM / YYYY-MM-DD) per the
// JSON Resume spec — presentation is altacv's `preferences.dateFormat`.
...
#let resume = from-json-resume((
  ...
  publications: (
    (name: "A Paper", type: "Articles", date: "2024"),
  ),
))
